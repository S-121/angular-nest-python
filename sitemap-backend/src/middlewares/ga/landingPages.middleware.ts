...
import { getLandingPages } from 'src/scripts';
import { Connection } from 'mongoose';
import { InjectConnection } from '@nestjs/mongoose';
import {
  GaUserSchema,
  LandingPagesSchema,
} from 'src/modules/google-analytics/schemas';
import { ProjectDocument } from 'src/modules/project/schemas';

@Injectable()
export class LandingPageMiddleware implements NestMiddleware {
  constructor(
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    ...
    if (!isCached) {
      const rows = await getLandingPages(req.user, viewId);
  ...
  async populateDB(collectionName, rows) {
    try {
      await this.connection.model(collectionName, LandingPagesSchema);
